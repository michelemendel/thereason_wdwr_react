...
let createOrder = (state): option(Shirt.Order.t) => {
  ...
  let result: option(Shirt.Order.t) =
    Some({
  ...
  render: self => {
    let orderItems =
      Belt.Array.map(
        ...
        order => {
          let beingEdited =
            switch (self.state.editingNumber) {
          ...
          <OrderItem
            ...
            order
            deleteFunction={_event => self.send(Delete(order))}
    ...
    <div>
      ...
      <p id="errorText"> {ReasonReact.string(self.state.errorText)} </p>
      orderTable
    </div>;
  },
};